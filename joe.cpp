...
joe::joe(float posX,float posY) : adversaire(posX,posY)
{
    m_sprite.setTexture(generalTexture::textureList["joe"]);
    m_sprite.setPosition(posX,posY);
    m_animMapTab = fonction::loadAnim("data/Animations/joeAnim.txt");
    m_etat = "run_left";
...
void joe::updateCollision(std::vector<plateforme>& plList,Vue& vue,std::vector<blocusAdv>& BadvList)
{
...
if(m_isAlive == true && this->m_isDying == false)
{
        ...
        this->m_sprite.setPosition(this->m_box.getPosX() + this->m_box.getSizeX()/2,this->m_box.getPosY());
...
std::vector<int> joe::update(hitBox& colBox,bool persoIsHiting,int persoDommage,hitBox& persoWeaponBox)
{
...
void joe::gravity(std::vector<plateforme>& obs)//même gestion de mouvement que pour le personnage principale
{
...
void joe::mouv(std::vector<plateforme>& obs)//même gestion de mouvement que pour le personnage principale
{
